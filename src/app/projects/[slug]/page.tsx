...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article>
    <header className="section border-b border-[var(--line)]"><div className="container max-w-5xl">
      ...
      <div className="mt-7 flex flex-wrap gap-2">{project.status && <span className="chip">{project.status}</span>}{project.projectType && <span className="chip">{project.projectType}</span>}{project.categories.map(category => <span className="chip" key={category}>{category}</span>)}</div>
    ...
    <div className="container grid max-w-6xl gap-12 py-14 lg:grid-cols-[minmax(0,1fr)_300px] lg:py-20">
      <div className="prose min-w-0">
        ...
        <section><h2>Models or techniques</h2><p>{project.analyticalApproach || "The analytical approach requires review before publication."}</p></section>
        <section><h2>Evaluation and results</h2>{project.results?.length ? <List items={project.results}/> : <div className="card my-4 flex gap-3 p-5"><TriangleAlert aria-hidden className="mt-1 shrink-0 text-[var(--accent)]"/><p className="!m-0">No verified result has been published in this portfolio.</p></div>}</section>
        ...
        <section><h2>Lessons learned</h2>{project.lessonsLearned?.length ? <List items={project.lessonsLearned}/> : <p>Separate lessons-learned statements are omitted until they can be verified; the limitations and future improvements above document the current learning boundary.</p>}</section>
        <section><h2>Technical evidence</h2><p>The public record includes the documented methodology, technology stack, evaluation notes, and the verified links provided with this case study.</p>{project.githubUrl && <p><a href={project.githubUrl} target="_blank" rel="noopener noreferrer">Inspect the source repository <ExternalLink aria-hidden className="inline" size={15}/></a></p>}</section>
      ...
      <aside className="h-fit min-w-0 lg:sticky lg:top-24">
        <div className="card p-6"><p className="eyebrow">Project record</p>{(project.year || project.projectType || project.role || project.status) && <dl className="mt-5 grid gap-3 text-sm">{project.year && <div><dt className="font-bold">Year</dt><dd className="mt-1 muted">{project.year}</dd></div>}{project.projectType && <div><dt className="font-bold">Project type</dt><dd className="mt-1 muted">{project.projectType}</dd></div>}{project.role && <div><dt className="font-bold">Role</dt><dd className="mt-1 muted">{project.role}</dd></div>}{project.status && <div><dt className="font-bold">Status</dt><dd className="mt-1 muted">{project.status}</dd></div>}</dl>}<h2 className="mt-7 font-black">Technologies</h2><div className="mt-3 flex flex-wrap gap-2">{project.technologies.map(item => <span className="chip" key={item}>{item}</span>)}</div><h2 className="mt-7 font-black">Verified links</h2><div className="mt-3 grid gap-2">{project.liveUrl ? <a className="btn btn-primary !w-full" href={project.liveUrl} target="_blank" rel="noopener noreferrer" aria-label={`Launch ${project.title} on Streamlit in a new tab`}>Live Streamlit app <ExternalLink aria-hidden size={15}/></a> : <p className="text-sm muted">Live application link requires review.</p>}{project.githubUrl ? <a className="btn !w-full" href={project.githubUrl} target="_blank" rel="noopener noreferrer" aria-label={`Open GitHub repository for ${project.title} in a new tab`}>GitHub repository <ExternalLink aria-hidden size={15}/></a> : <p className="text-sm muted">Repository link requires review.</p>}</div>{project.liveUrl && <p className="mt-4 text-xs leading-5 muted">Streamlit Community Cloud applications may take a few moments to start after a period of inactivity.</p>}</div>
